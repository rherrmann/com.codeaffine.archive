...
if [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "master" ]; then
  ...
  git clone --quiet --branch=gh-pages https://${GH_TOKEN}@github.com/rherrmann/com.codeaffine.archive.git . > /dev/null 2>&1 || error_exit "Error cloning gh-pages"
  ...
  git add -f .
  git commit -m "[ci skip] [skip ci] Deploy Travis build number $TRAVIS_BUILD_NUMBER to gh-pages"
